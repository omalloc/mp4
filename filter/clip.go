...
func (f *clipFilter) FilterMoov(m *box.MoovBox) error {
	if f.err != nil {
		return f.err
	}
...
func (f *clipFilter) buildChunkList(tnum int, t *box.TrakBox) {
...
func (f *clipFilter) updateSamples(tnum int, t *box.TrakBox) {
...
func (f *clipFilter) updateChunks(tnum int, t *box.TrakBox) {
...
func (f *clipFilter) updateChunkOffsets(m *box.MoovBox, deltaOff int) uint32 {
	stco, i := make([]*box.StcoBox, len(m.Trak)), make([]int, len(m.Trak))
	for tnum, t := range m.Trak {
...
func (f *clipFilter) updateDurations(m *box.MoovBox) {
...
func (f *clipFilter) FilterMdat(w io.Writer, m *box.MdatBox) error {
	if f.err != nil {
		return f.err
	}
